    ...
    <main className="hospital-detail-page" style={{ paddingTop: 'var(--navbar-height)' }}>
      ...
      <section style={{ position: 'relative', minHeight: 380, display: 'flex', alignItems: 'flex-end', overflow: 'hidden' }}>
        ...
        <div style={{
          position: 'absolute',
          inset: 0,
          background: 'linear-gradient(to top, rgba(9,13,16,0.85) 0%, rgba(9,13,16,0.40) 60%, rgba(9,13,16,0.20) 100%)',
        }} />
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', marginBottom: '1rem', flex: 1 }}>
                        {sp.procedures.slice(0, 2).map((proc) => (
                          <div key={proc.id} style={{ display: 'flex', alignItems: 'center', gap: '0.45rem', fontSize: '0.8125rem' }}>
                            <span style={{ color: 'var(--color-primary)' }}>✦</span>
                            <span style={{ color: 'var(--color-text-secondary)', fontWeight: 500 }}>{l(proc, 'name')}</span>
                          </div>
                        ))}
                      </div>
                      <Link
