...
import { Star, MapPin, Shield, Edit3, Eye } from 'lucide-react';
...
const HOSPITAL_FIELDS: FieldDefinition<Hospital>[] = [
  ...
  { key: 'specialties', label: 'Specialty IDs (e.g. sp-cardiology, sp-oncology)', type: 'array' },
...
export function AdminHospitalsPage() {
  ...
  const getInitialHospital = (): Hospital => ({
    ...
    specialties: ['sp-cardiology', 'sp-oncology'],
    bedsCount: 500,
    icuBeds: 50,
    foundedYear: 2000,
    rating: 4.8,
  ...
  return (
    <AdminEntityManager<Hospital>
      ...
      subtitle="Manage accredited healthcare facilities, bed counts, and clinical accreditations"
      entityName="Hospital"
      items={hospitals}
      loading={loading}
      ...
      renderTableColumns={['Hospital', 'Location', 'Rating', 'Beds & Capacity', 'Accreditations', 'Actions']}
      renderTableRow={(h, onEdit, onView, isSelected, onToggleSelect) => (
        ...
        >
          ...
          <td style={{ padding: '0.875rem 0.85rem', fontSize: '0.8125rem' }}>
            <div><strong>{formatNumber(h.bedsCount)}</strong> Beds</div>
            <div style={{ fontSize: '0.72rem', color: 'var(--color-text-muted)' }}>{formatNumber(h.internationalPatientsPerYear)} intl/yr</div>
          </td>
          <td style={{ padding: '0.875rem 0.85rem' }}>
